...
## Player 2 movement controller for the swimming minigame.
## Handles alternating button swimming mechanics with bumpers only.
## P2: L1/R1 bumpers on gamepad 1

const OrganConst = preload("res://scripts/core/organ_constants.gd")

@export var player_index: int = 1  # 0 for P1, 1 for P2
...
const FRICTION: float = 0.8                 # Velocity damping
...
func _physics_process(delta: float) -> void:
	...
	if debug_enabled and frame_count % 60 == 0:
		print("P2 frame: ", frame_count, " last_btn: ", last_button_pressed, " vel.y: ", velocity.y)
...
func _update_alternation_mechanic(delta: float) -> void:
	...
	var button_a_pressed = _is_p2_button_a_pressed()
	var button_b_pressed = _is_p2_button_b_pressed()
